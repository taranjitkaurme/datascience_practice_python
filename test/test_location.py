...
def test_create_location(app):
    ...
    with app.app_context():
        new_location_result = LocationService.create_location(city="Test City", state="Test State", country="Test "
                                                                                                            "Country")
        assert "message" in new_location_result[0]
        assert new_location_result[0]["message"] == "Location created successfully"
...
def test_get_location(app):
    ...
    with app.app_context():
        ...
        assert "message" in non_existing_location_info[0]
        assert non_existing_location_info[0]["message"] == "Location not found"


def test_update_location(app):
    # Test update_location with existing location
    with app.app_context():
        update_existing_result = LocationService.update_location(location_id=1, city="Updated City", state="Updated "
                                                                                                           "State",
        ...
        update_non_existing_result = LocationService.update_location(location_id=100, city="Updated City",
                                                                     state="Updated State", country="Updated Country")
        assert "message" in update_non_existing_result[0]
        assert update_non_existing_result[0]["message"] == "Location not found"
...
def test_delete_location(app):
    ...
    with app.app_context():
        ...
        assert delete_non_existing_result[0]["message"] == "Location not found"
