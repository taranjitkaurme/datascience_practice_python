...
def test_get_company(app):  # Pass the session fixture
    with app.app_context():
        ...
        print(company_info)
        assert "company_id" in company_info[0]
        assert "name" in company_info[1]
...
def test_update_company(app):
    ...
    with app.app_context():
        ...
        assert "message" in update_existing_result[0]
        assert update_existing_result[0]["message"] == "Company updated successfully"
...
def test_delete_company(app):
    ...
    with app.app_context():
        ...
        assert "message" in delete_existing_result[0]
        assert delete_existing_result[0]["message"] == "Company deleted successfully"
